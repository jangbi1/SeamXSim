...
def test(params):
    ...
    for s in tqdm(range(num_test_samples)):
        if args.dataset == 'kitti':
            ...
            filename_pred_png = save_name + '/raw/' + lines[s].split()[0].split('/')[-1].replace('.png', '.png')
            filename_cmap_png = save_name + '/cmap/' + lines[s].split()[0].split('/')[-1].replace('.png', '.png')
            filename_image_png = save_name + '/rgb/' + lines[s].split()[0].split('/')[-1]
        else:
            scene_name = lines[s].split()[0].split('/')[0]
#             print(scene_name)
#             print(save_name)
            filename_pred_png = save_name + '/raw/' + '_' + lines[s].split()[0].split('/')[-1].replace(
                '.png', '.png')
            print(filename_pred_png)

            filename_cmap_png = save_name + '/cmap/' + '_' + lines[s].split()[0].replace(
                '.png', '.png')
            filename_gt_png = save_name + '/gt/' + '_' + lines[s].split()[0].replace(
                '.png', '.png')
            filename_image_png = save_name + '/rgb/' + '_' + lines[s].split()[0]
        ...
        if args.dataset == 'nyu':
            ...
            # print(gt_path)
            gt = cv2.imread(gt_path).astype(np.float32) / 255.0  # Visualization purpose only
            gt = cv2.resize(gt, dsize=(320,320), interpolation=cv2.INTER_LINEAR)
            gt[gt == 0] = np.amax(gt)
        ...
        if args.dataset == 'kitti' or args.dataset == 'kitti_benchmark':
            ...
            pred_depth_scaled = pred_depth * 255.0
        
        pred_depth_scaled = pred_depth_scaled.astype(np.float32)

        cv2.imwrite(filename_pred_png, pred_depth_scaled, [cv2.IMWRITE_PNG_COMPRESSION, 100])
