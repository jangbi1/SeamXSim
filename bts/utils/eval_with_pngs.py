...
parser.add_argument('--max_depth_eval',      type=float, help='maximum depth for evaluation', default=1)
...
def test():
    ...
    for i in range(num_test_samples):
        ...
        if pred_depth.ndim != 2:
            pred_depth = pred_depth[:,:,0:3]
            pred_depth = cv2.cvtColor(pred_depth, cv2.COLOR_BGR2GRAY)
        ...
        if args.dataset == 'nyu':
            pred_depth = pred_depth.astype(np.float32) / 255.0
#         else:
#             pred_depth = pred_depth.astype(np.float32) / 256.0
    ...
    if args.dataset == 'kitti':
        ...
        for t_id in range(num_test_samples):
            ...
            directory = directory[1:].zfill(8) + '_depth' ######## Scenario 
            gt_depth_path = os.path.join(args.gt_path, directory + '.png')
            gt_depth_path = gt_depth_path.replace('_image','depth') # Unity dataset
            gt_depth_path = gt_depth_path.replace('_FrameBuffer','Depth') # colon_depth
            # print(gt_depth_path)
            
            depth = cv2.imread(gt_depth_path, -1)
            # depth = cv2.resize(depth, dsize=(320,320))
            # break
            if depth.ndim != 2:
                depth = depth[:,:,0:3]
            ...
            # depth_norm = (depth - depth_min) / depth_scale
            # inv_depth = 1.0 - depth_norm
            # depth = depth_min + (inv_depth * depth_scale)
            ###############
            gt_depths.append(depth)
...
#         print(pred_depth.min())
#         print('pred_depth end\n')
#         break
...
#         print(valid_mask.shape)
    ...
    # pandas DataFrame을 사용하여 CSV 파일에 저장
