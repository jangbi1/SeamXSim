...
def generate_images(
    ...
    Generate images using pretrained network pickle.
    """
    seed = 4  # pick up a random number
    random.seed(seed)
    np.random.seed(seed)
    ...
    with torch.no_grad():
        for k, ipath in enumerate(img_list):
            input = np.zeros([512,512,3], dtype='uint8')
            ...
            # input = image[:,:,:3]
            # # Normal 
            input[256-128:256+128,256-128:256+128,:] = image[0:256,0:256,:3]
            ...
            PIL.Image.fromarray(output, 'RGB').save(f'{outdir}/pad_{iname}')
